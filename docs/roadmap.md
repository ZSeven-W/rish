...
- [x] Bun 1.4.0 musl baseline 实际路径所需 SSE3/SSSE3/SSE4.1 子集（`--version` 与 JavaScript 执行均已通过；不宣称完整 SSE4.1/4.2）
...
- [x] guest-side init/systemd/cgroup/namespace/runtime evidence probe 与条件式 OCI lifecycle dispatch
- [ ] 局部 APIC（LAPIC timer/EOI）boot 路径验证与 I/O APIC、ACPI 表
- [ ] x86_64 Linux kernel/initramfs 可复现构建
- [ ] OCI lifecycle worker/async response、verified rootfs import 与完整 guest agent handler
